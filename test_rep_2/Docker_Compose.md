 ...
 **핵심은 “셋업은 한 번에, 역할은 분리해서”** 가는 거다. 저희의 프로젝트는 Docker Compose 기반으로 Spring, Python, MySQL, Redis가 함께 뜨는 구조라서, 팀원별 실행 방법과 담당 범위 md파일로 분명히 적어 온보딩 하게 되었습니다.[^1]
...
## 내가 추천하는 실제 역할 분담
 ...
 지금 Docker/환경 셋업을 끝냈으니까, **팀 리드처럼 공통 환경 유지 + Python AI 파트**, 팀원 A는 Spring API, 팀원 C는 프론트로 가는 게 가장 자연스럽다. 이 구조면 서로 병목이 적고, mock 모드 덕분에 AI가 늦어져도 전체 개발은 계속 갈 수 있다.
